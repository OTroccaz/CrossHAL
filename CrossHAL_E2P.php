 ...
 *
 * Version prototype - Prototype version
 */
        ...
        if ($lienMAJ != "") {
          ...
          if ($actMaj == "ok") {
            ...
            if ($submDate <= $limDate) {//Il est possible de faire les modifications car la date limite est dépassée
              $textAff .= "<center><span id='maj".$halID."'><a target='_blank' href='".$lienMAJ."' onclick='$.post(\"CrossHAL_liste_actions.php\", { halID: \"".$halID."\", action: \"".$actsMAJ."\" });majok(\"".$halID."\");'><img alt='MAJ' style='width: 50px;' src='./img/add_grand.png'></a></span></center>";
            }else{
              $textAff .= "<center><img alt='Embargo' title='Embargo: interdit de modifier cette notice car la date \"whenSubmitted\" < 90 jours' style='width: 50px;' src='./img/addEmbargo_grand.png'></center>";
            }
          }else{
      ...
      if($ordAut == "oui") {
        ...
        if ($doi != "") {
          ...
          //if (($autHALTot == $autCRTot) && (count($arrayCR["message"]["author"]) == count($arrayHAL["response"]["docs"][$cpt]["authLastName_s"]))) {
            ...
            if ($actMaj == "ok") {
              $textAff .= "<center><span id='maj".$halID."'><a target='_blank' href='".$lienMAJAut."' onclick='$.post(\"CrossHAL_liste_actions.php\", { halID: \"".$halID."\", action: \"MAJ_AUT\" });majok(\"".$halID."\");'><img alt='MAJ' style='width: 50px;' src='./img/add_grand.png'></a></span></center>";
            }else{
          ...
          if (normalize(strtolower(wd_remove_accents($prenomsHAL))) != normalize(strtolower(wd_remove_accents($prenomsCR))) && $prenomsCR != "" && preg_match("/^[a-zA-Z]+\.|[a-zA-Z], [a-zA-Z]+\./", $prenomsCR) != 1 && $nbPreHAL == $nbPreCR) {
            ...
            if ($lienMAJPre != "") {
              ...
              if ($actMaj == "ok") {
                ...
                $limDate = time() - $nbjours*86400;
                if ($submDate <= $limDate) {//Il est possible de faire les modifications car la date limite est dépassée
                  $textAff .= "<center><span id='maj".$halID."'><a target='_blank' href='".$lienMAJPre."' onclick='$.post(\"CrossHAL_liste_actions.php\", { halID: \"".$halID."\", action: \"MAJ_PRE\" });majok(\"".$halID."\");'><img alt='MAJ' style='width: 50px;' src='./img/add_grand.png'></a></span></center>";
                }else{
                  $textAff .= "<center><img alt='Embargo' title='Embargo: interdit de modifier cette notice car la date \"whenSubmitted\" < 90 jours' style='width: 50px;' src='./img/addEmbargo_grand.png'></center>";
                }
              }else{
            ...
            if (array_search($tabIdHAL["nc"][$cpt], $tabIdHALsNC) !== false) {//...mais peut-être qu'un IdHAL a déjà été trouvé pour cet auteur dans une autre notice
              ...
              if ($iHALtst == "ok") {
                ...
                if ($actMaj == "ok") {
                  ...
                  if ($submDate <= $limDate) {//Il est possible de faire les modifications car la date limite est dépassée
                    $lignAff = "ok";
                    $textAff .= "<td><center><span id='maj".$halID."'><a target='_blank' href='".$lienIDH."' onclick='$.post(\"CrossHAL_liste_actions.php\", { halID: \"".$halID."\", action: \"MAJ_IDH\" });majok(\"".$halID."\");'><img alt='MAJ' style='width: 50px;' src='./img/add_grand.png'></a></span></center></td>";
                  }else{
                    $lignAff = "ok";
                    $textAff .= "<td><center><img alt='Embargo' title='Embargo: interdit de modifier cette notice car la date \"whenSubmitted\" < 90 jours' style='width: 50px;' src='./img/addEmbargo_grand.png'></center></td>";
                  }
                }else{
                  $lignAff = "ok";
        ...
        if ($actMaj == "ok") {
          ...
          if ($submDate <= $limDate) {//Il est possible de faire les modifications car la date limite est dépassée
            if ($condAct == "ok") {//Il y a une condition préalable au lancement de l'action
              $textAff .= "<td><center><span id='maj".$halID."'><img alt='MAJ' title='Par précaution, ce bouton Action ne sera activé que lorsque vous aurez vérifié via le lien ci-avant que le PDF est bien un manuscrit auteur' style='width: 50px;' src='./img/addOK_grand.png'></span></center></td>";
            }else{
              $textAff .= "<td><center><span id='maj".$halID."'><a target='_blank' href='".$lienPDF."' onclick='$.post(\"CrossHAL_liste_actions.php\", { halID: \"".$halID."\", action: \"MAJ_PDF\" });majok(\"".$halID."\");'><img alt='MAJ' style='width: 50px;' src='./img/add_grand.png'></a></span></center></td>";
            }
          }else{
            $textAff .= "<td><center><img alt='Embargo' title='Embargo: interdit de modifier cette notice car la date \"whenSubmitted\" < 90 jours' style='width: 50px;' src='./img/addEmbargo_grand.png'></center></td>";
          }
        ...
        if ($action == "3" && $halID == $getHalID) {
          ...
          if ($lienext == "oui") {//notice avec lien externe
            ...
            if ($actMaj == "ok") {
              ...
              $limDate = time() - $nbjours*86400;
              if ($submDate <= $limDate) {//Il est possible de faire les modifications car la date limite est dépassée
                $textAff .= "<center><span id='maj".$halID."'><a target='_blank' href='".$lienPDF."' onclick='$.post(\"CrossHAL_liste_actions.php\", { halID: \"".$halID."\", action: \"MAJ_PDF\" });majok(\"".$halID."\");'><img alt='MAJ' style='width: 50px;' src='./img/add_grand.png'></a></span></center>";
              }else{
                $textAff .= "<center><img alt='Embargo' title='Embargo: interdit de modifier cette notice car la date \"whenSubmitted\" < 90 jours' style='width: 50px;' src='./img/addEmbargo_grand.png'></center>";
              }
              $txtAff = "ok";
            ...
            if ($lienPDF == "noDateEpub") {
              $textAff .= "<center><img alt='Pas de dateEpub' title=\"La date de publication en ligne n'est pas renseignée !\" style='width: 50px;' src='./img/addEmbargo_grand.png'></center>";
              $lignAff = "ok";
            }else{
              if ($actMaj == "ok") {
                $textAff .= "<center><span id='maj".$halID."'><a target='_blank' href='".$lienPDF."' onclick='$.post(\"CrossHAL_liste_actions.php\", { halID: \"".$halID."\", action: \"MAJ_PDF\" });majok(\"".$halID."\");'><img alt='MAJ' style='width: 50px;' src='./img/add_grand.png'></a></span></center>";
